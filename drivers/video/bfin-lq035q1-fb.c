...
static int bfin_lq035q1_fb_mmap(struct fb_info *info, struct vm_area_struct *vma)
{
	...
	vma->vm_start = (unsigned long)(fbi->fb_buffer + fbi->active_video_mem_offset);
...
static int __devinit bfin_lq035q1_probe(struct platform_device *pdev)
{
	...
	if (ret < 0) {
		printk(KERN_ERR DRIVER_NAME "Failed PPI Mode\n");
		goto out3;
	}
	...
	info->active_video_mem_offset = ((U_LINE / 2) *
				LCD_X_RES * (info->lcd_bpp / 8));

	fbinfo->fix.smem_len = LCD_X_RES * LCD_Y_RES * info->lcd_bpp / 8
				+ info->active_video_mem_offset;
	...
	printk(KERN_INFO DRIVER_NAME ": %dx%d %d-bit RGB FrameBuffer initialized\n",
